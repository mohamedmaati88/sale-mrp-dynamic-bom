    ...
    def action_confirm(self):
        ...
        for line in valid_lines:
            if line.is_mandatory and not line.is_selected:
                raise UserError(
                    'Component "%s" is mandatory and cannot be deselected.'
        ...
        if max_qty_errors:
            raise UserError(
                'The following components exceed their maximum allowed quantity:\n\n%s'
                % '\n'.join(max_qty_errors)
            )
        ...
        for group in seen_groups:
            ...
            if group.is_required and not selected:
                raise UserError(
                    'At least one component must be selected from required '
                    'group "%s".' % group.name
                )
            if group.selection_mode == 'single' and len(selected) > 1:
                raise UserError(
                    'Only one component can be selected from single-selection '
        ...
        if incompat_errors:
            raise UserError(
                'The following selected components are incompatible and '
        ...
        if exact_match_line:
            raise UserError(
                'Product "%s" already exists in this sale order with the exact same '
        ...
        if not components_vals:
            raise UserError(
                'No components were selected. Please select at least one component.'
            )
        ...
        sale_line.order_id.message_post(
            body='Components configured for product <b>%s</b> (line %s): '
