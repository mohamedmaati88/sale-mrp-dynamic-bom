...
class SaleOrder(models.Model):
    ...
    def _compute_price_adjustment_pending(self):
        ...
        for order in self:
            ...
            for line in order.order_line.filtered(
                ...
                if not posted_lines and isinstance(line.id, int):
    ...
    def action_view_dynamic_component_lines(self):
        ...
        _MrpProd = self.env['mrp.production']
        _has_group = 'procurement_group_id' in _MrpProd._fields
        _group = _has_group and getattr(self, 'procurement_group_id', False)
        if _has_group and _group:
            _mo_domain = [
                ('is_dynamic_bom', '=', True),
                mo_state_domain,
                '|',
                ('origin', '=', self.name),
                ('procurement_group_id', '=', _group.id),
            ]
        else:
            _mo_domain = [
                ('is_dynamic_bom', '=', True),
        ...
        confirmed_mo = _MrpProd.search(_mo_domain, limit=1)
        ctx = {
    ...
    def action_update_components_on_mo(self):
        ...
        _has_group = 'procurement_group_id' in MrpProduction._fields
        _group = _has_group and getattr(self, 'procurement_group_id', False)
        if _has_group and _group:
            domain = [
                ('is_dynamic_bom', '=', True),
        ...
        if not productions:
            raise UserError(_('No dynamic manufacturing order found for this sale order.'))
        ...
        if blocked:
            ...
            ) % (block_msg, ', '.join(blocked.mapped('name'))))
        ...
        for production in productions.filtered(lambda p: p.state in allowed_states):
            ...
            # ── Rebuild BOM lines and raw moves ────────────────────────
            bom = production.bom_id
            if not bom:
                continue
            sale_line._rebuild_mo_bom_and_moves(
                production, bom, sale_line.selected_component_ids
            )

            updated.append(production.name)
            comp_lines = Markup('<br/>').join(
                Markup('• %s × %.2f %s') % (
            ...
            production.message_post(
                body=Markup(
                    '🔄 <b>Dynamic BOM updated</b> from sale order <b>%s</b> '
                    'with <b>%d</b> component(s):<br/>%s'
                ) % (self.name, len(sale_line.selected_component_ids), comp_lines)
            )

        if not updated:
            raise UserError(_('No draft manufacturing orders were found to update.'))
    ...
    def action_update_all_lines(self):
        ...
        if errors and not updated:
            raise UserError(_('Could not update any line:\n%s') % '\n'.join(errors))
    ...
    def action_cancel(self):
        ...
        # Collect all dynamic MOs so we can clean their activities after super()
        all_dynamic_mos = self.env['mrp.production'].sudo()

        for order in to_process:
            _MrpProd = self.env['mrp.production'].sudo()
            _has_group = 'procurement_group_id' in _MrpProd._fields
            _group = _has_group and getattr(order, 'procurement_group_id', False)
            if _has_group and _group:
                _cancel_domain = [
                    ('is_dynamic_bom', '=', True),
                    ('state', 'not in', ['cancel']),
            ...
            dynamic_mos = _MrpProd.search(_cancel_domain)
            if not dynamic_mos:
                continue

            all_dynamic_mos |= dynamic_mos

            # draft → cancel automatically
            auto_cancel = dynamic_mos.filtered(lambda p: p.state == 'draft')
            # confirmed → mark cancelled, post chatter message
            confirmed_warn = dynamic_mos.filtered(lambda p: p.state == 'confirmed')
            # in-progress / done → skip, warn on SO
        ...
        # Remove ALL activities on dynamic MOs created/re-created by super().action_cancel()
        # (Odoo stock/procurement auto-creates exception activities during SO cancellation)
        if all_dynamic_mos:
            all_dynamic_mos.sudo().activity_ids.unlink()

        # Remove all activities on the cancelled SO itself
    ...
    def _setup_dynamic_bom_on_productions(self):
        ...
        for order in self:
            ...
            group = getattr(order, 'procurement_group_id', False)
            mrp_has_group = 'procurement_group_id' in MrpProduction._fields
            if mrp_has_group and group:
                domain = [
            ...
            for production in productions:
                ...
                bom_line_vals = sale_line._bom_line_vals_for_components(
                    sale_line.selected_component_ids
                )
                ...
                _bom_created_body = Markup(
                    '🔧 <b>Dynamic BOM</b> created from '
                    '<b>%s</b> with <b>%d</b> component(s):<br/>%s'
                ) % (order.name, len(bom_line_vals), comp_lines)
                production.message_post(body=_bom_created_body)
                new_bom.sudo().message_post(body=_bom_created_body)
...
class SaleOrderLine(models.Model):
    ...
    def action_view_components(self):
        ...
        MrpProduction = self.env['mrp.production']
        mrp_has_group = 'procurement_group_id' in MrpProduction._fields
        group = mrp_has_group and getattr(self.order_id, 'procurement_group_id', False)
        if mrp_has_group and group:
            mo_search_domain = [
                ('is_dynamic_bom', '=', True),
        ...
        confirmed_mo = MrpProduction.search(mo_search_domain, limit=1)
        ctx = {
    ...
    def _bom_line_vals_for_components(self, components):
        """Return list of (0,0,{...}) BOM line vals for the given components.
        Single source of truth used by all three BOM-building paths."""
        return [(0, 0, {
    ...
    def _rebuild_mo_bom_and_moves(self, production, bom, components):
        """Rebuild BOM lines and raw moves for a manufacturing order.
        Single source of truth used by all three BOM-building paths."""
        bom.sudo().bom_line_ids.unlink()
        bom.sudo().write({
            'product_qty': 1,
            'bom_line_ids': self._bom_line_vals_for_components(components),
        })
        production.sudo().move_raw_ids.filtered(
            lambda m: m.state not in ('done', 'cancel')
        ).unlink()
        ...
        # Post component list on the BOM chatter
        order_name = self.order_id.name if self.order_id else (bom.code or '')
        comp_lines = Markup('<br/>').join(
            Markup('• %s × %.2f %s') % (
                c.component_product_id.display_name,
                c.qty,
        ...
        _bom_updated_body = Markup(
            '🔄 <b>Dynamic BOM updated</b> from sale order <b>%s</b> '
            'with <b>%d</b> component(s):<br/>%s'
        ) % (order_name, len(components), comp_lines)
        bom.sudo().message_post(body=_bom_updated_body)
    ...
    def action_update_components(self):
        ...
        if not matching:
            raise UserError(
                _('No dynamic manufacturing order found for "%s".')
        ...
        if blocked:
            raise UserError(
                _('Manufacturing order "%s" is %s and cannot be updated.')
        ...
        bom = production.bom_id
        if not bom:
            raise UserError(_('No BOM found for manufacturing order "%s".') % production.name)

        # Rebuild BOM lines and raw moves
        self._rebuild_mo_bom_and_moves(production, bom, self.selected_component_ids)
        ...
        production.message_post(
            body=Markup(
                '🔄 <b>Dynamic BOM updated</b> from sale order <b>%s</b> '
                'with <b>%d</b> component(s):<br/>%s'
            ) % (order.name, len(self.selected_component_ids), comp_lines)
        )
    ...
    def _sync_dynamic_mo_quantity(self, new_qty):
        ...
        mrp_has_group = 'procurement_group_id' in MrpProduction._fields
        group = mrp_has_group and getattr(order, 'procurement_group_id', False)
        if mrp_has_group and group:
            mo_domain = [
                ('is_dynamic_bom', '=', True),
                ('state', 'not in', ['done', 'cancel']),
        ...
        productions = MrpProduction.search(mo_domain)
        ...
        main_mo.message_post(
            body=Markup('📦 <b>Quantity updated to %.2f</b> from Sale Order <b>%s</b>.') % (new_qty, order.name)
        )
    ...
    def _check_mo_not_confirmed(self):
        ...
        if lock_mode == 'on_mo_confirm':
            ...
            err_msg = _('The manufacturing order "%s" is already confirmed. '
                        'You cannot make changes after confirmation.')
        elif lock_mode == 'manual':
            state_domain = ('lock_component_updates', '=', True)
            err_msg = _('The manufacturing order "%s" is manually locked by the '
                        'production operator. No updates are allowed.')
        else:
            state_domain = ('state', '=', 'done')
            err_msg = _('The manufacturing order "%s" is already closed (Done). '
    ...
    def _get_income_account(self):
        """Return the income account for this SO line's product.

        Tries product-level account first, then category-level, then falls
        back to the first income account on the sale journal — compatible
        with both Odoo 16 (property fields) and Odoo 17/19 (company-dependent).
        """
        ...
        if account:
            return account
        # Fallback: ask account.move.line to compute it via the product
        journal = self.env['account.journal'].search([
            ('type', '=', 'sale'),
            ('company_id', '=', self.order_id.company_id.id),
        ], limit=1)
        if journal:
            dummy_line = self.env['account.move.line'].new({
            ...
            dummy_line._compute_account_id()
            if dummy_line.account_id:
                return dummy_line.account_id
        raise UserError(
            _("No income account found for product '%s'. "
    ...
    def _adjust_invoices_for_price_update(self, new_price):
        ...
        def _find_inv_lines(state, move_type):
            ...
            if not lines and isinstance(self.id, int):
                lines = AML.search([
        ...
        def _create_diff_snap(source, qty_val, stype):
            ...
            source is either a sale.order.line.component or a snapshot record."""
            pkg_qty = (source.packaging_qty or 1.0) if hasattr(source, 'packaging_qty') else 1.0
            # Use effective price (respects pricing_mode / components_cost)
            if hasattr(source, '_effective_unit_price'):
                effective_price = source._effective_unit_price()
            else:
                effective_price = source.sale_price or 0.0
            Snapshot.create({
                ...
                'subtotal': effective_price * qty_val * pkg_qty,
                'notes': source.notes or '',
                'snapshot_type': stype,
                'sequence': source.sequence if source.sequence else 10,
    ...
    def _validate_component_compatibility(self):
        ...
        for line in self.filtered(
            ...
            if errors:
                raise UserError(
                    _('Product "%s": the following selected components are '
    ...
    def _check_dynamic_components_configured(self):
        for line in self:
            ...
            if not line.selected_component_ids:
                raise UserError(
                    _('Please configure components for product "%s" before '
